...
/** Tavern 概览 KPI —— 仅 usage_logs 可统计项（无 TTFT）。 */
import { computed } from 'vue';

import { BILLING_FRACTION_DIGITS, formatMoney } from '@/shared/lib/money';
import type { LogStats } from '@demux/common';
import { shortNumber } from '@/features/demux/components/overview/chartUtils';
  ...
  <div class="kpi-grid">
    ...
    <div class="kpi-card">
      ...
      <div class="kpi-card__value">
        {{ formatMoney(stats?.totalCost ?? 0, { fractionDigits: BILLING_FRACTION_DIGITS }) }}
      </div>
    ...
    <div class="kpi-card">
      ...
      <div class="kpi-card__sub">
        {{ (stats?.totalTokens ?? 0).toLocaleString() }} tokens
      </div>
    </div>
  </div>
</template>
