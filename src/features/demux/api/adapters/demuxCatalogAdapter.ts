...
import type { DemuxCatalogPort } from '@/features/demux/services/ports/demuxCatalogPort';
...
export class DemuxCatalogHttpAdapter implements DemuxCatalogPort {
  async listProviderGroups(): Promise<AppResult<ProviderGroup[]>> {
    const res = await requestDemux<ItemsEnvelope<unknown>>(`${BASE}/catalog/groups`);
    if (!res.success) return res;
    return parseGroups(res.data);
  }

  async listUpstreamModels(queueGroup: string): Promise<AppResult<ProviderUpstreamModel[]>> {
    const res = await requestDemux<ItemsEnvelope<unknown>>(`${BASE}/${encodeURIComponent(queueGroup)}/models`);
    if (!res.success) return res;
    return parseModels(res.data);
  }

  async discoverFromGateway(): Promise<AppResult<DiscoverCatalogResult>> {
    const res = await requestDemux<unknown>(`${BASE}/discovery`);
    if (!res.success) return res;
  ...
  async importProviderGroup(
    ...
    const res = await requestDemux<unknown>(`${BASE}/import`, {
      method: 'POST',
      body: {
  ...
  async deleteProviderGroup(id: string): Promise<AppResult<void>> {
    return requestDemux<void>(`${BASE}/${encodeURIComponent(id)}`, { method: 'DELETE' });
  }

  async deleteUpstreamModel(queueGroup: string, modelId: string): Promise<AppResult<void>> {
    return requestDemux<void>(
  ...
  async updateVendorSlug(id: string, vendorSlug: string | null): Promise<AppResult<void>> {
    return requestDemux<void>(`${BASE}/${encodeURIComponent(id)}/vendor/slug`, {
      method: 'PATCH',
      body: { vendorSlug },
    });
  }
