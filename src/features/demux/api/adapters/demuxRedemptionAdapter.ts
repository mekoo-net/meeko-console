...
} from '@/features/demux/services/ports/demuxRedemptionPort';
...
export class DemuxRedemptionHttpAdapter implements DemuxRedemptionPort {
  async list(input: {
    page: number;
    pageSize: number;
    ...
    const result = await requestDemux<ItemsEnvelope<RedemptionDtoRaw>>(path, { query });
  ...
  async create(input: CreateRedemptionCodesInput): Promise<AppResult<CreateRedemptionCodesResult>> {
    ...
    const result = await requestDemux<CreateRedemptionsResponseRaw>(BASE, {
      method: 'POST',
      body: {
  ...
  async remove(id: number): Promise<AppResult<void>> {
    return requestDemux<void>(`${BASE}/${id}`, { method: 'DELETE' });
  }
