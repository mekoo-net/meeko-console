...
function mapRawItem(raw: unknown): LogEntry {
  ...
  if (!parsed.success) {
    ...
    console.warn('[DemuxLogsAdapter] parse error', parsed.error.issues[0], remapped);
...
export class DemuxLogsHttpAdapter implements DemuxLogsPort {
  async list(input: {
    page: number;
    pageSize: number;
    filter: ListLogsFilter;
    ...
    const result = await requestDemux<ItemsEnvelope<unknown>>(BASE, {
      query: {
        ...
        logId:      filter.logId      || undefined,
        traceId:    filter.traceId    || undefined,
        billSerialNo: filter.billUid  || undefined,
        contactKeyword: filter.contactKeyword?.trim() || undefined,
  ...
  async statByVendor(filter: ListLogsFilter): Promise<AppResult<VendorConsumptionRow[]>> {
    const result = await requestDemux<ItemsEnvelope<VendorRow>>(`${BASE}/stats/by/vendor`, {
      query: {
        fromUtc:    filter.fromUtc,
        toUtc:      filter.toUtc,
        accountUid: filter.accountUid || undefined,
  ...
  async stats(filter: ListLogsFilter): Promise<AppResult<LogStats>> {
    ...
    const [bucketResult, modelResult, providerResult, errorCodeResult, latencyResult] = await Promise.all([
      requestDemux<ItemsEnvelope<BucketRow>>(`${BASE}/stats`, { query }),
      requestDemux<ItemsEnvelope<ModelRankRow>>(`${BASE}/stats/by/model`, { query }),
      requestDemux<ItemsEnvelope<ProviderRankRow>>(`${BASE}/stats/by/provider`, { query }),
      requestDemux<ItemsEnvelope<ErrorCodeRow>>(`${BASE}/stats/by/errorcode`, { query }),
      requestDemux<LatencyStat>(`${BASE}/stats/latency`, { query }),
    ]);
