...
let cached: DemuxStore | null = null;

/** 模块级单例：第一次调用时建表 + 种子；避免不同 Port 看到不同视图。 */
export function getDemuxStore(): DemuxStore {
...
/** 仅供单测复位用；运行时不调用。 */
export function _resetDemuxStore(): void {
  cached = null;
}
...
function collectAliveModels(store: DemuxStore): Map<string, ProviderModel> {
...
export function reconcilePlatformModels(store: DemuxStore): ReconcileDiff {
