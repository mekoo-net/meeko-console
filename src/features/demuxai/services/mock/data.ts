...
function seedProviders(): Provider[] {
  ...
  const openaiMain: Provider = {
    ...
    modelMappings: [
      mp(o_gpt4o.uid, 'GPT-4o', { order: 0 }),
      mp(o_gpt4omini.uid, 'GPT-4o mini', { order: 10 }),
      ...
      mp(o_o1mini.uid, 'o1-mini', { order: 20 }),
      mp(o_emb3.uid, 'Text-Embedding-3-Large', { order: 30 }),
    ],
  ...
  const azureNA: Provider = {
    ...
    modelMappings: [
      mp(a_gpt4o.uid, 'GPT-4o', { order: 0 }),
      mp(a_gpt4omini.uid, 'GPT-4o mini', { order: 10 }),
    ],
  ...
  const anthropic: Provider = {
    ...
    modelMappings: [
      mp(an_sonnet.uid, 'Claude 3.5 Sonnet', { order: 0 }),
      mp(an_haiku.uid, 'Claude 3.5 Haiku', { order: 10 }),
    ],
  ...
  const aliyun: Provider = {
    ...
    modelMappings: [
      mp(ali_max.uid, 'Qwen-Max', { order: 0 }),
      mp(ali_plus.uid, 'Qwen-Plus', { order: 10 }),
    ],
  ...
  const deepseek: Provider = {
    ...
    modelMappings: [
      mp(ds_chat.uid, 'DeepSeek-V3', { order: 0 }),
      mp(ds_reasoner.uid, 'DeepSeek-R1 (推理)', { order: 10 }),
    ],
  ...
  const selfHosted: Provider = {
    ...
    modelMappings: [mp(sh_llama.uid, 'Llama 3.1 70B (自建)', { order: 0 })],
  ...
  const moonshot: Provider = {
    ...
    modelMappings: [mp(ms_32k.uid, 'Moonshot 32K', { order: 0 })],
  ...
  const gemini: Provider = {
    ...
    modelMappings: [
      mp(g_flash.uid, 'Gemini 1.5 Flash', { order: 0, notes: '灰度' }),
    ],
